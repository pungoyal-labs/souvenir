 ...
 * the pies they won. Deletion vacates their seat, so they are found where the
 * record still names them: the ledger, the markets they opened, their table
 * talk, and the bills they are on. `membersOf` is the live roster.
 */
async function membersById(tripId: string): Promise<Map<string, Member>> {
  const onRecord = union(
    ...
    db.select({ id: ledger.memberId }).from(ledger).where(eq(ledger.tripId, tripId)),
    db.select({ id: markets.creatorId }).from(markets).where(eq(markets.tripId, tripId)),
    db
    ...
    db
      .select({ id: billRevisions.editorId })
      .from(billRevisions)
      .innerJoin(bills, eq(bills.id, billRevisions.billId))
      .where(eq(bills.tripId, tripId)),
    db
      .select({ id: billEntries.memberId })
      .from(billEntries)
      .innerJoin(billRevisions, eq(billRevisions.id, billEntries.revisionId))
      .innerJoin(bills, eq(bills.id, billRevisions.billId))
      .where(eq(bills.tripId, tripId)),
  );
...
export async function deleteAccount(memberId: string): Promise<void> {
  await db.transaction(async (tx) => {
    // Deleting an account vacates every seat at once, so the same rule as
    // setRole applies to each trip they organise: never down to no organiser
    // while anyone else is still seated. Rows are locked for the count so a
    // concurrent demotion cannot slip past it.
...
/**
 * What the member a link names must not be able to miss: a live link to their
 * own seat, or one walked through recently. The members-page notice assumes
 * that page gets opened; the person with the most at stake may never open it
 * inside a link's 30-minute life, so the root layout follows them with this
 * on every page.
 */
 ...
 */
/** Validation before any write, so a refused revision leaves nothing behind. */
function checkRevision(trip: Trip, input: BillInput, opts?: { deleted?: boolean }) {
  ...
  return { checked, entryValues, deleted: opts?.deleted ?? false };
}
...
async function insertRevision(
  ...
  editorId: string,
  billId: string,
  { checked, entryValues, deleted }: ReturnType<typeof checkRevision>,
): Promise<void> {
...
async function appendRevision(
  ...
  const revision = checkRevision(trip, input, opts);
  await db.transaction(async (tx) => {
    ...
    await insertRevision(tx, trip, editorId, billId, revision);
  });
}
...
export async function addBill(tripId: string, memberId: string, input: BillInput): Promise<string> {
  ...
  const revision = checkRevision(trip, input);
  const id = randomUUID();
  // One transaction: a refused first revision must not leave an orphan bill.
  await db.transaction(async (tx) => {
    await tx.insert(bills).values({ id, tripId });
    await insertRevision(tx, trip, memberId, id, revision);
  });
