...
export default async function MembersPage() {
  ...
  const [{ ranked, unranked }, invites, legacy, passkeys, recoveries, { balances }] =
    await Promise.all([
      leaderboard(),
      listInvites(),
      listAllowlist(),
  ...
  const joined = new Set(all.map((s) => s.member.email).filter((e) => e != null));
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[11px] text-soft">
              ...
              {s.member.isFounder && <span className="text-gold"> · founder</span>}
            </p>
          </div>
        </div>
      </td>
