...
-- would have given it — counting up past slugs already in the trip, since
-- "taxi-2" may itself exist (uniqueSlug minted those per member).
DO $$
DECLARE
	r record;
	n integer;
BEGIN
	FOR r IN
		SELECT p."id", p."trip_id", p."slug"
		FROM "phrases" p
		JOIN (
			SELECT "id", row_number() OVER (PARTITION BY "trip_id", "slug" ORDER BY "created_at", "id") AS rn
			FROM "phrases"
		) d ON d."id" = p."id"
		WHERE d.rn > 1
	...
	LOOP
		...
		WHILE EXISTS (
			...
			WHERE "trip_id" = r."trip_id" AND "slug" = r."slug" || '-' || n AND "id" <> r."id"
		) LOOP
			n := n + 1;
		END LOOP;
		UPDATE "phrases" SET "slug" = r."slug" || '-' || n WHERE "id" = r."id";
	END LOOP;
